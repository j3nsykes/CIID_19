...
void draw() {
  ...
  if (data != null) {
    ...
    try {
      //get one face
      //get one element . Try get different elements depending on the number in the array box. 
      ...
      for (int j = 0; j < coordinates.size(); j++) { //some landmarks have multiple points. Get them all
        ...
        x = map(array.getFloat(0), 0, camWidth, 0, width); //get the x co-ordinate
        y = map(array.getFloat(1), 0, camHeight, 0, height); //get the y co-ordinate
        ellipse(x, y, 5, 5);
